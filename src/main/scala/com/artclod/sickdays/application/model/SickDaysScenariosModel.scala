...
class SickDaysScenariosModel(initialScenarios : Iterable[SickDaysScenarioModel] = new HashSet) {
	...
	def newScenario() = scenarios.add(new SickDaysScenarioModel(setupValue = new SickDaysModel(durationValue = 10, virusValue = new VirusModel, locationsValue = new LocationModel) ) )
